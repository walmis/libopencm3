...
void adc_set_injected_sequence(uint32_t adc, uint8_t length, uint8_t channel[])
{
  ...
  for (i = 0; i < length; i++) {
    reg32 |= ADC_JSQR_JSQ_VAL(i+1, channel[i]);
  }
...
void adc_set_injected_offset(uint32_t adc, uint8_t reg, uint32_t offset)
{
  ...
  case 1:
    ...
    ADC_OFR1(adc) |= offset;
    ADC_OFR1(adc) |= ((ADC_JSQR(adc) >> ADC_JSQR_JSQ1_LSB) & 0x1F) << 26;
    break;
  case 2:
    ADC_OFR2(adc) = 0;
    ADC_OFR2(adc) |= ADC_OFR2_OFFSET2_EN;
    ADC_OFR2(adc) |= offset;
    ADC_OFR2(adc) |= ((ADC_JSQR(adc) >> ADC_JSQR_JSQ2_LSB) & 0x1F) << 26;
    break;
  case 3:
	ADC_OFR3(adc) = 0;
    ADC_OFR3(adc) |= ADC_OFR3_OFFSET3_EN;
    ADC_OFR3(adc) |= offset;
    ADC_OFR3(adc) |= ((ADC_JSQR(adc) >> ADC_JSQR_JSQ3_LSB) & 0x1F) << 26;
    break;
  case 4:
	ADC_OFR4(adc) = 0;
    ADC_OFR4(adc) |= ADC_OFR4_OFFSET4_EN;
    ADC_OFR4(adc) |= offset;
    ADC_OFR4(adc) |= ((ADC_JSQR(adc) >> ADC_JSQR_JSQ4_LSB) & 0x1F) << 26;
